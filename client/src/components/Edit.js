  ...
  return ( 
    ...
    : (<>
        ...
        <div className="linksList">
          {(links.length > 0) ? (links.map(link => <EditableLink key={link._id} id={link._id} link={link} username={username} rerender={rerender}/>)) : (<h1>Your ManyLinks is empty, add a link and try again.</h1>)}
        </div>
      </>)
  )
}
